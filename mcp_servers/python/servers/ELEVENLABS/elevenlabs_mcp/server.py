...
# Remove mandatory env var requirement – api_key can now be supplied per-call via __credentials__

# Add global custom client placeholder; will be set in get_client()
...
def get_client(credentials: dict | None = None) -> ElevenLabs:
    """Return an ElevenLabs client, creating one if necessary.

    If `credentials` is provided, always create a fresh client using that key.
    Otherwise, reuse the global client initialised from env-var (if present).
    ...
    # Fall back to env var if no credentials provided
    if not key:
        key = api_key
    
    if key is None or key == "":
        raise ValueError("ElevenLabs API key missing – pass it via credentials.apiKey")

    # Always create a fresh client when credentials are provided
    if credentials and isinstance(credentials, dict):
        return ElevenLabs(api_key=key, httpx_client=custom_client)

    # Lazy-initialise global client if not yet created
    if client is None:
        client = ElevenLabs(api_key=key, httpx_client=custom_client)
    return client
...
def text_to_speech(
    ...
    if voice_id is not None:
        voice = get_client(credentials).voices.get(voice_id=voice_id)
    elif voice_name is not None:
        voices = get_client(credentials).voices.get_all()
    ...
    audio_data = get_client(credentials).text_to_speech.convert(
        text=text,
        voice_id=voice_id,
        model_id=model_id,
...
def speech_to_text(
    ...
    with file_path.open("rb") as f:
        audio_bytes = f.read()
    transcription = get_client(credentials).speech_to_text.convert(
        model_id="scribe_v1",
        file=audio_bytes,
...
def text_to_sound_effects(
    ...
    audio_data = get_client(credentials).text_to_sound_effects.convert(
        text=text,
        output_format=output_format,
...
def search_voices(
    ...
    response = get_client(credentials).voices.get_all()
    voices = response.voices
    
    # Filter by search term if provided
    if search:
        voices = [v for v in voices if search.lower() in v.name.lower()]
    
    # Sort voices
    if sort == "name":
        voices.sort(key=lambda x: x.name, reverse=(sort_direction == "desc"))
    elif sort == "created_at_unix":
        voices.sort(key=lambda x: getattr(x, 'created_at_unix', 0), reverse=(sort_direction == "desc"))
...
def list_models(credentials: dict | None = None) -> list[McpModel]:
    response = get_client(credentials).models.list()
    return [
        McpModel(
            id=model.model_id,
            name=model.name,
...
def get_voice(voice_id: str, credentials: dict | None = None) -> McpVoice:
    """Get details of a specific voice."""
    response = get_client(credentials).voices.get(voice_id=voice_id)
    return McpVoice(
...
def voice_clone(
    name: str, files: list[str], description: str | None = None, credentials: dict | None = None
) -> TextContent:
    input_files = [str(handle_input_file(file).absolute()) for file in files]
    voice = get_client(credentials).voices.ivc.create(
...
def isolate_audio(
    input_file_path: str, output_directory: str | None = None, credentials: dict | None = None
) -> list[TextContent]:
    file_path = handle_input_file(input_file_path)
    output_path = make_output_path(output_directory, base_path)
    ...
    with file_path.open("rb") as f:
        audio_bytes = f.read()
    audio_data = get_client(credentials).audio_isolation.convert(
        audio=audio_bytes,
    )
    audio_bytes = b"".join(audio_data)
...
def check_subscription(credentials: dict | None = None) -> TextContent:
    subscription = get_client(credentials).user.subscription.get()
...
def create_agent(
    ...
    response = get_client(credentials).conversational_ai.agents.create(
        name=name,
...
def add_knowledge_base_to_agent(
    ...
    if url is not None:
        response = get_client(credentials).conversational_ai.knowledge_base.documents.create_from_url(
        ...
        response = get_client(credentials).conversational_ai.knowledge_base.documents.create_from_file(
            name=knowledge_base_name,
            file=file,
        )

    agent = get_client(credentials).conversational_ai.agents.get(agent_id=agent_id)
    ...
    get_client(credentials).conversational_ai.agents.update(
...
def list_agents(credentials: dict | None = None) -> TextContent:
...
def get_agent(agent_id: str, credentials: dict | None = None) -> TextContent:
    ...
    """
    response = get_client(credentials).conversational_ai.agents.get(agent_id=agent_id)
...
def get_conversation(
    ...
    try:
        response = get_client(credentials).conversational_ai.conversations.get(conversation_id)
...
def list_conversations(
    ...
    try:
        response = get_client(credentials).conversational_ai.conversations.list(
            cursor=cursor,
            agent_id=agent_id,
...
def speech_to_speech(
    ...
    audio_data = get_client(credentials).speech_to_speech.convert(
...
def text_to_voice(
    ...
    previews = get_client(credentials).text_to_voice.create_previews(
...
def create_voice_from_preview(
    ...
    voice = get_client(credentials).text_to_voice.create_voice_from_preview(
...
def _get_phone_number_by_id(phone_number_id: str, credentials: dict | None = None):
    """Helper function to get phone number details by ID."""
    phone_numbers = get_client(credentials).conversational_ai.phone_numbers.list()
...
def make_outbound_call(
    ...
    phone_number = _get_phone_number_by_id(agent_phone_number_id, credentials)
    
    if phone_number.provider.lower() == "twilio":
        response = get_client(credentials).conversational_ai.twilio.outbound_call(
            agent_id=agent_id,
        ...
        response = get_client(credentials).conversational_ai.sip_trunk.outbound_call(
            agent_id=agent_id,
            agent_phone_number_id=agent_phone_number_id,
            to_number=to_number,
        )
...
def search_voice_library(
    ...
    response = get_client(credentials).voices.get_shared(
        page=page,
        page_size=page_size,
        search=search,
    )
...
def list_phone_numbers(credentials: dict | None = None) -> TextContent:
    """List all phone numbers associated with the ElevenLabs account.

    Returns:
        TextContent containing formatted information about the phone numbers
    """
    response = get_client(credentials).conversational_ai.phone_numbers.list()
...
def play_audio(input_file_path: str, credentials: dict | None = None) -> TextContent:
    file_path = handle_input_file(input_file_path)
