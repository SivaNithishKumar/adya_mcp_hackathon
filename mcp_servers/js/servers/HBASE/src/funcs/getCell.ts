...
export async function getCellTool(client: HBaseCoreClient, params: { __credentials__?: { url?: string; port?: number }; table?: string; row?: string; column?: string }) {
  ...
  return await client.getCell({ url: creds.url, port: creds.port, table: params.table, row: params.row, column: params.column });
} 