...
export async function deleteTableTool(client: HBaseCoreClient, params: { __credentials__?: { url?: string; port?: number }; table?: string }) {
  ...
  return await client.deleteTable({ url: creds.url, port: creds.port, table: params.table });
} 