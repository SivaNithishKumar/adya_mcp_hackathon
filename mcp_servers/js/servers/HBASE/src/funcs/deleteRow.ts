...
//   row: string - Row key to delete
import { HBaseCoreClient } from '../core.js';

export async function deleteRowTool(client: HBaseCoreClient, params: { __credentials__?: { url?: string; port?: number }; table?: string; row?: string }) {
  ...
  return await client.deleteRow({ url: creds.url, port: creds.port, table: params.table, row: params.row });
} 