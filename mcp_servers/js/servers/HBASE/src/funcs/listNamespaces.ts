...
export async function listNamespacesTool(client: HBaseCoreClient, params: { __credentials__?: { url?: string; port?: number } }) {
  ...
  return await client.listNamespaces({ url: creds.url, port: creds.port });
} 