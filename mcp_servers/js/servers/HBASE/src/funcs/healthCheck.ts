...
export async function healthCheckTool(client: HBaseCoreClient, params: { __credentials__?: { url?: string; port?: number } }) {
  ...
  return await client.healthCheck({ url: creds.url, port: creds.port });
} 