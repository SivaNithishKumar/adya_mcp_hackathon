...
//   region: string - Region name to compact
import { HBaseCoreClient } from '../core.js';

export async function compactRegionTool(client: HBaseCoreClient, params: { __credentials__?: { url?: string; port?: number }; table?: string; region?: string }) {
  ...
  return await client.compactRegion({ url: creds.url, port: creds.port, table: params.table, region: params.region });
} 